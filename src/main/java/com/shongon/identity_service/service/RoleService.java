...
public class RoleService {
    ...
    public CreateRoleResponse createRole(CreateRoleRequest request) {
        if (roleRepo.existsById(request.getRole_name())) throw new AppException(ErrorCode.ROLE_EXISTED);
    ...
    public UpdateRoleResponse updateRolePermissions(String role_name, UpdateRoleRequest request) {
        Role role = roleRepo.findById(role_name).orElseThrow(() -> new AppException(ErrorCode.ROLE_NOT_FOUND));
    ...
    public void deleteRole(String role_name) {
        if (!roleRepo.existsById(role_name)) throw new AppException(ErrorCode.ROLE_NOT_FOUND);
